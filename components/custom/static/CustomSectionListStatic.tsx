...
import { DescriptionRenderer } from '@/components/ui/description-renderer';
...
function ListItem({ primaryTitle, secondaryTitle, dateInfo, location, description, logoUrl, customLinks, isFirst = false, isLast = false }: ListItemProps) {
  // Add conditional padding: pt-6 if not first, pb-6 if not last
  ...
  return (
    <li className={paddingClasses}>
      <div className="flex gap-4">
        ...
        {description && (
          <DescriptionRenderer description={description} className="prose text-md dark:prose-invert" />
        )}
