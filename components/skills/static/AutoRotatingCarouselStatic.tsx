...
export default function AutoRotatingCarouselStatic({ skills }: StaticSkillsData) {
  // Separate skills with logos from those without (data is pre-processed)
  ...
  // Calculate total pages needed based on responsive items per page
  ...
  // Auto-rotate through technologies (matching original)
  useEffect(() => {
  ...
  return (
    <section id="skills" className="mb-28">
      ...
      {/* Horizontal sliding carousel layout for technologies with logos (matching original) */}
      {skillsWithLogos.length > 0 && (
        <div className="mb-8">
          <div className="flex justify-center items-center">
            <div className="relative w-full overflow-hidden">
              {/* Main carousel container with sliding animation */}
              <div
                className="flex transition-transform duration-700 ease-in-out"
                style={{ transform: `translateX(-${currentIndex * 100}%)` }}
              >
                {/* Generate pages (matching original structure) */}
                  ...
                  return (
                    <div key={`page-${pageIndex}`} className="w-full flex-shrink-0 flex justify-center">
                      <div className="flex justify-between items-center w-full max-w-2xl sm:max-w-4xl md:max-w-5xl px-4">
                        ...
                        {/* Fill empty slots if less than max items (matching original) */}
              ...
              {/* Navigation dots (matching original) */}
              {totalPages > 1 && (
      ...
      {/* Other Technologies section - only for technologies without logos */}
